...
//   const MultiplayerScreenActivity({Key? key, this.gameKey, this.firstTry, this.oppornentName, this.oppornentPic, this.round, this.imagex, this.imageo}) : super(key: key);
...
//                 gotcoin: entryfee! * 2,
//                 oppornentId: player1 == _auth.currentUser!.uid ? player2 : player1,
...
//               Dialoge.winner(context, winnerId == _auth.currentUser!.uid ? username : "${utils.limitChar(widget.oppornentName, 15)}",
//                   winnerId == _auth.currentUser!.uid ? profilePic : "${widget.oppornentPic}", winText, point, widget.gameKey);

//               var _tempData = (await _gameSnapshot)!.snapshot.value;

//               if (winnerId == _auth.currentUser!.uid) {
//                 History()
//                     .update(uid: winnerId, date: DateTime.now().toString(), gameid: widget.gameKey, gotcoin: (_tempData as Map)["entryFee"] * 2, oppornentId: looserId, status: "Won", type: "GAME");
...
//               History().update(
//                   uid: idOfPlayer2, date: DateTime.now().toString(), gameid: widget.gameKey, gotcoin: (_tempData as Map)["entryFee"], oppornentId: idOfPlayer1, status: "Tie", type: "TIE GAME");
//               //looser's history update
...
//             Dialoge.winner(context, winnerId == _auth.currentUser!.uid ? username : "${utils.limitChar(widget.oppornentName, 15)}",
//                 winnerId == _auth.currentUser!.uid ? profilePic : "${widget.oppornentPic}", winText, earnedCoin, widget.gameKey);
...
//                         },
//                         child: Container(
//                           decoration: BoxDecoration(image: DecorationImage(image: AssetImage("assets/images/grid_box.png"), fit: BoxFit.fill)),
//                           child: buttons[i] == null || buttons[i]['state'] == "" ? Container() : Image.asset(returnImage(i)),
//                         ),
//                       );
//                     },
//                   ),
            ...
            if (winnerId != "") {
              ...
              if (winnerId == _auth.currentUser.uid) {
                History()
                    .update(uid: winnerId, date: DateTime.now().toString(), gameid: widget.gameKey, gotcoin: (_tempData as Map)["entryFee"] * 2, oppornentId: looserId, status: "Won", type: "GAME");
                //looser's history update
                History().update(uid: looserId, date: DateTime.now().toString(), gameid: widget.gameKey, gotcoin: (_tempData as Map)["entryFee"], oppornentId: winnerId, status: "Lose", type: "GAME");
          ...
          if (winnerId == "") {
            ...
            if (idOfPlayer1 == _auth.currentUser.uid) {
              History().update(
                  uid: idOfPlayer2, date: DateTime.now().toString(), gameid: widget.gameKey, gotcoin: (_tempData as Map)["entryFee"], oppornentId: idOfPlayer1, status: "Tie", type: "TIE GAME");
              //looser's history update
              History().update(uid: idOfPlayer1, date: DateTime.now().toString(), gameid: widget.gameKey, gotcoin: (_tempData as Map)["entryFee"], oppornentId: idOfPlayer2, status: "Tie", type: "TIE GAME");
