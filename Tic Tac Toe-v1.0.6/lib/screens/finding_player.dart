...
// class _FindingPlayerScreenState extends State<FindingPlayerScreen> with ChangeNotifier {
...
//   String oppMsg = findingOpp, img = "assets/images/dora_findopponent.png", btnTxt = "Cancel";
...
//     DatabaseEvent oppornentDetail = await _ins.ref().child("users").child(key).once();
...
//           //change listener
//           listen = _ins.ref().child("Game").child(data["roomKey"]).onChildChanged.listen((DatabaseEvent ev) async {
//             if (ev.snapshot.key == "status" && ev.snapshot.value != "closed" && ev.snapshot.value != "pending") {
...
//               DatabaseEvent _player2snap = await _ins.ref().child("Game").child(data["roomKey"]).child("player2").once();
...
//                 var getFirstTry = await _ins.ref().child("Game").child(data["roomKey"]).once();
//                 firstTry = (getFirstTry.snapshot.value as Map)["try"];

//                 var getFirstTryId = await _ins.ref().child("Game").child(data["roomKey"]).child(firstTry!).child("id").once();
...
//     DatabaseEvent coinOld = await _ins.ref().child("users").child(_auth.currentUser!.uid).once();
//     var fin = (coinOld.snapshot.value as Map)["coin"] - widget.selected;
//     _ins.ref().child("users").child(_auth.currentUser!.uid).update({"coin": fin});
//     //--
//   }
...
//   canPlay(key) async {
//     var _player1 = await _ins.ref().child("Game").child(key).child("player1").once();
//     var _player2 = await _ins.ref().child("Game").child(key).child("player2").once();

//     var player1 = (_player1.snapshot.value as Map)["id"];
//     var player2 = (_player2.snapshot.value as Map)["id"];

//     if (player1 == FirebaseAuth.instance.currentUser!.uid || player2 == FirebaseAuth.instance.currentUser!.uid) {
//       oppositPlayerName.notifyListeners();
//       canUpdateUi = true;
//       changeScreen(context);
//     } else {
...
//                                           child: CircleAvatar(radius: 50, backgroundColor: secondaryColor, backgroundImage: _profilePic == null ? null : NetworkImage(_profilePic!)))),
//                                   SizedBox(
//                                     height: 5,
//                                   ),
//                                 ],
//                               ),
//                             ),
//                             Expanded(flex: 2, child: Image.asset("assets/images/vs_iconbig.png")),
//                             Expanded(
//                                 flex: 4,
//                                 child: Column(
//                                   children: [
...
//                                             backgroundImage: oppositPlayerName.value != "" && canUpdateUi == true ? NetworkImage("$_opporentPic") : null,
//                                             child: oppositPlayerName.value != "" && canUpdateUi == true
//                                                 ? null
//                                                 : Center(
//                                                     child: Text(
//                                                     "?",
...
//                                 child: Text(
//                                   oppositPlayerName.value != "" && canUpdateUi == true ? "${oppositPlayerName.value} \n" : "$opponentPlayerName \n",
//                                   style: TextStyle(color: white),
//                                   textAlign: TextAlign.center,
//                                   maxLines: 2,
//                                   softWrap: true,
...
//                                 decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), color: back),
//                                 child: Row(
//                                   mainAxisSize: MainAxisSize.min,
//                                   children: [
//                                     Icon(
//                                       btnTxt == utils.getTranslated(context, "tryAgain") ? Icons.replay_circle_filled : Icons.cancel,
//                                       color: primaryColor,
//                                     ),
//                                     Text(
//                                       btnTxt,
...
//                                 )),
//                             onPressed: () {
//                               if (btnTxt == utils.getTranslated(context, "tryAgain")) {
//                                 setState(() {
//                                   oppMsg = utils.getTranslated(context, "findingOpp");
//                                   opponentPlayerName = utils.getTranslated(context, "waitForOpponent");
...
//                                   });
//                                 });
//                               } else if (btnTxt == utils.getTranslated(context, "cancel")) {
//                                 if (_temp != "") {
//                                   FirebaseDatabase.instance.ref().child("Game").child(_temp!).update({"status": "closed"});
//                                   Dialoge.removeChild("Game", _temp);
//                                 }
//                                 oppTimer!.cancel();
//                                 Navigator.pop(context);
...
class _FindingPlayerScreenState extends State<FindingPlayerScreen> with ChangeNotifier {
  ...
  StreamSubscription<DatabaseEvent> listen;
   ValueNotifier oppositPlayerName;
   ValueNotifier keyOfGame;
  bool canPlayGame = false;
  bool isplaying = false;
  bool canUpdateUi = false;
  bool isCoinAndCountValueUpdated = false;
